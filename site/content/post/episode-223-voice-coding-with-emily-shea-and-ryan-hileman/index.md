...
MARK: And speaking of incredibly smooth segues and giving the world access to everything, we actually, for our Helpful Tip of the Week, I wanted to ask Bret McGowen about how to build an API on Google Cloud. This is something that we had talked about many, many weeks ago, but I just didn't get a chance to include it up until now. So why don't we turn things over to Bret as he gives us our helpful tip of the week. 

[MUSIC PLAYING] 

BRET: Hi, I'm Bret McGowen, and I am a serverless developer advocate for Google Cloud. There are a bunch of great ways to build APIs with Google Cloud. In fact, my teammate Martin and I just had a few videos in our serverless tool box series on building REST APIs from scratch with Cloud Run. We actually did a video that uses Google Sheets as its data store, which I think is kind of cool, because everyone in your organization probably already knows how to use Google Sheets. You can control who gets to view and edit the Sheets. You can actually have business folks editing and updating data in those Sheets using the mobile app or on their computer, without you having to build an admin user interface into your database. You can check those how-to videos out at serverlesstoolbox.com. 

And now I have two tips for getting started building APIs on Google Cloud. The first tip is, try using our serverless platforms like Cloud Run and Cloud Functions. They're easy to use, and they do a great job with these kind of request-response models that APIs often use. So say I'm calling an API, and I'm sending some JSON in the body of my HTTP request, and I want to get back some response from the server. 

They're a good way to get started because they're fully managed, which means, as a developer, you're pretty much just concerned about your code and your business logic. You don't have to set up or manage the infrastructure. You don't have to provision capacity. You don't have to predict scaling. You don't have to set up networking. You don't have to do load balancing-- yada, yada. A lot of this stuff is taken care of for you. 

But what's particularly great about serverless is that it scales. So it literally scales, like as traffic comes in and out, but also in the sense of scaling with your app as you build it. It's "pay for what you use." So it's much, much cheaper when you're just developing an app or you have low traffic. And additionally, it's already highly available, it's already resilient. So you can go from quick proof of concept, to MVP, to version one of your production app. And you can stay on the same platform. It grows and scales with you. 

Now, choosing between Cloud Functions and Cloud Run comes down to one technical difference and one philosophical difference. The technical difference is around containers. Cloud Run uses containers, which means you can use any programming language, any libraries. You just package it up into a container and deploy it. Cloud Functions can actually be even easier, because you just apply your source code. You don't have to create a container. But the trade-off is that it only supports certain languages-- so Node.js, Python and Go, but we're working on adding other languages. 

Now, the philosophical difference between Cloud Functions and Cloud Run is that Cloud Functions is intended for single-purpose services. So let's say you have an HTTP REST API. A good design would be either one cloud function that handles the post-- the GET/DELETE requests for one specific entity, say, widgets-- or you could actually have four different Cloud Functions for each entity, one that handles GET widget, one that handles POST widget, one for PUT, one for DELETE widget. This is a really nice pattern, and one I like a lot, because each function is nice and simple. And if you deploy an update to one of these functions, it doesn't touch or potentially break any of the other functions in your API. 

But what you probably don't want to do is create one function where you cram in an entire REST API for widgets, and users, and profiles, and articles, all in one function. If you do actually want to have your entire API in one app-- or maybe you've already got an existing API that looks like that, so an API app using Flask in Python, or Express in Node, Spring in Java, you can just package that whole thing up into a container, and use Cloud Run. 

Plus what's nice is Cloud Run has traffic splitting. So you can use A/B testing. And it supports concurrency, which means, for each instance of your app, it can handle multiple requests simultaneously. And if one instance starts to get overloaded, then Cloud Run will automatically create a new instance of your app, and start sending traffic there. 

My second tip around building APIs that I recommend using a layer in front of your actual API code. So this is often called an API gateway. Basically it's something that just receives incoming requests, looks at the URL, and then routes it to some appropriate back-end service. Now, the reason that this is useful is because you can change where your code actually runs without changing any external-facing URLs. You can split a service from one Cloud Run service, and turn it into five Cloud Functions. And none of your external URLs have to change, so your users aren't disrupted or affected. 

Now, two tools that play nice with serverless that do this kind of API gateway functionality is Firebase Hosting on the simple side. It's super simple. It has a JSON file where you just say, this URL maps to this Cloud Run service, or it maps to this Cloud Function. And then on the advanced end is Apogee, which is like enterprise-grade-level API management. It does URL mapping, and also authentication. You can route to on-premise services, it can create a portal for your developers, like, a whole bunch of stuff. 

So if you're creating APIs using serverless on Google Cloud, I would love to answer your questions and hear about your experience. Happy building. 

MARK: Thanks again, Bret. Super helpful. And of course, we'll put a link in the show nets to "Serverless Toolbox," which is the YouTube series he and Martin do on just a wide variety of some of serverless stuff on Google Cloud. 

BRIAN: OK, thank you Bret, and thank you, everyone, for listening to the whole episode. And be safe out there. Take care, and see you next week. 
